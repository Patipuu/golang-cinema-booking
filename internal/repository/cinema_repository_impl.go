...
func (r *CinemaRepositoryImpl) FindByID(ctx context.Context, id string) (*domain.Cinema, error) {
    var c domain.Cinema
    err := r.db.Pool.QueryRow(ctx, `SELECT id, name, location, city, hotline, created_at, updated_at FROM cinemas WHERE id = $1`, id).Scan(
        &c.ID, &c.Name, &c.Location, &c.City, &c.Hotline, &c.CreatedAt, &c.UpdatedAt,
    )
    if err != nil {
        return nil, err
    }
...
func (r *CinemaRepositoryImpl) ListAll(ctx context.Context, page domain.Page) ([]domain.Cinema, domain.PageResult, error) {
    rows, err := r.db.Pool.Query(ctx, `SELECT id, name, location, city, hotline, created_at, updated_at FROM cinemas`)
    if err != nil {
    ...
    for rows.Next() {
        var c domain.Cinema
        if err := rows.Scan(&c.ID, &c.Name, &c.Location, &c.City, &c.Hotline, &c.CreatedAt, &c.UpdatedAt); err != nil {
            return nil, domain.PageResult{}, err
        }
...
func (r *CinemaRepositoryImpl) ListShowtimes(ctx context.Context, cinemaID string, date time.Time) ([]domain.Showtime, error) {
    rows, err := r.db.Pool.Query(ctx, `SELECT id, cinema_id, movie_id, room_id, show_date, show_time, price, created_at FROM showtimes WHERE cinema_id = $1 AND show_date = $2`, cinemaID, date)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (r *CinemaRepositoryImpl) ListSeatsByCinema(ctx context.Context, cinemaID string) ([]domain.Seat, error) {
    rows, err := r.db.Pool.Query(ctx, `
        SELECT s.id, s.room_id, s.seat_number, s.seat_type 
        FROM seats s 
        JOIN screening_rooms r ON s.room_id = r.id 
        WHERE r.cinema_id = $1`, cinemaID)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
    ...
    for rows.Next() {
        var s domain.Seat
        if err := rows.Scan(&s.ID, &s.RoomID, &s.SeatNumber, &s.SeatType); err != nil {
            return nil, err
        }
