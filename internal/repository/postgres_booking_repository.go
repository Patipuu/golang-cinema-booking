...
func (r *PostgresBookingRepository) Create(ctx context.Context, booking *domain.Booking, seatCodes []string) error {
	...
	log.Printf("Locked seats for booking: %v", lockedSeatIDs)
	log.Printf("Seat codes requested: %v", seatCodes)	
	log.Printf("SeatID by code: %v", seatIDByCode)
	log.Printf("ShowtimeID: %s, RoomID: %s", booking.ShowtimeID, roomID)
	log.Printf("Price per seat: %.2f", pricePerSeat)
