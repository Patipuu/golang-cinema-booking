...
func (r *ShowtimeRepositoryImpl) FindByID(ctx context.Context, id string) (*domain.Showtime, error) {
    query := `SELECT id, cinema_id, movie_id, room_id, show_date, show_time, price, created_at FROM showtimes WHERE id = $1`
    var s domain.Showtime
    err := r.db.Pool.QueryRow(ctx, query, id).Scan(
        &s.ID, &s.CinemaID, &s.MovieID, &s.RoomID, &s.ShowDate, &s.ShowTime, &s.Price, &s.CreatedAt,
    )
    if err != nil {
        return nil, err
    }
...
func (r *ShowtimeRepositoryImpl) FindByCinema(ctx context.Context, cinemaID string, date time.Time) ([]*domain.Showtime, error) {
    query := `SELECT id, cinema_id, movie_id, room_id, show_date, show_time, price, created_at FROM showtimes WHERE cinema_id = $1 AND show_date = $2 ORDER BY show_time`
    ...
    for rows.Next() {
        var s domain.Showtime
        if err := rows.Scan(&s.ID, &s.CinemaID, &s.MovieID, &s.RoomID, &s.ShowDate, &s.ShowTime, &s.Price, &s.CreatedAt); err != nil {
            return nil, err
        }
...
func (r *ShowtimeRepositoryImpl) Create(ctx context.Context, showtime *domain.Showtime) error {
    query := `INSERT INTO showtimes (id, cinema_id, movie_id, room_id, show_date, show_time, price, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
    _, err := r.db.Pool.Exec(ctx, query,
        showtime.ID, showtime.CinemaID, showtime.MovieID, showtime.RoomID, showtime.ShowDate, showtime.ShowTime, showtime.Price, showtime.CreatedAt,
    )
    return err
}

func (r *ShowtimeRepositoryImpl) Update(ctx context.Context, showtime *domain.Showtime) error {
    query := `UPDATE showtimes SET cinema_id = $1, movie_id = $2, room_id = $3, show_date = $4, show_time = $5, price = $6 WHERE id = $7`
    _, err := r.db.Pool.Exec(ctx, query,
        showtime.CinemaID, showtime.MovieID, showtime.RoomID, showtime.ShowDate, showtime.ShowTime, showtime.Price, showtime.ID,
    )
    return err
}
...
func (r *ShowtimeRepositoryImpl) SearchShowtimes(ctx context.Context, movieName, cinemaID string, date *time.Time) ([]*domain.ShowtimeWithMovie, error) {
    ...
    query := `SELECT s.id, s.cinema_id, s.movie_id, s.room_id, s.show_date, s.show_time, s.price, s.created_at,
                     m.id, m.title_vi, m.duration_mins, m.rating_label, m.created_at
              FROM showtimes s
    ...
    idx := 1
    if movieName != "" {
        query += " AND LOWER(m.title_vi) LIKE $" + fmt.Sprint(idx)
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(
            &s.ID, &s.CinemaID, &s.MovieID, &s.RoomID, &s.ShowDate, &s.ShowTime, &s.Price, &s.CreatedAt,
            &m.ID, &m.TitleVI, &m.DurationMins, &m.RatingLabel, &m.CreatedAt,
        ); err != nil {
            return nil, err
        }
